#!/usr/bin/env python3
"""
Diagnostic: Split London V2 trades by top-scoring regime at entry time.

Same pattern as the V44 ambiguous split diagnostic — tag every London V2 trade
...
Looking for a "bleed bucket" where London V2 takes trades in a regime
that's structurally bad for breakout strategies.
"""
...
def run_diagnostic(dataset_label: str, input_csv: str, baseline_json: str) -> dict:
    print(f"\n{'='*70}")
    print(f"  LONDON V2 REGIME AUDIT: {dataset_label}")
    print(f"{'='*70}")
    ...
    print(f"Total London V2 trades: {len(v2_trades)}")
    ...
    if ambiguous_trades:
        ...
        for regime in sorted(ambig_by_top.keys()):
            _summarize(f"AMBIGUOUS, TOP={regime.upper()}", ambig_by_top[regime])

    return {
        "total_trades": len(v2_trades),
...
def main() -> int:
    ...
    out_path = ROOT / "research_out" / "diagnostic_london_v2_regime_split.json"
