#!/usr/bin/env python3
"""Run Phase3 v7_pfdd defended simulation — pipeline L1 spread (0.3 pip) in the inner runner."""
...
def main() -> None:
    ...
    output_dir = ROOT / "research_out/phase3_v7_pfdd_defended_zero"
    ...
    log("=== Phase3 v7_pfdd defended — pipeline L1 spread (runner) ===")
    log(f"Started at {time.strftime('%Y-%m-%d %H:%M:%S')} UTC")

    cfg = RunConfig(
        hypothesis="phase3_v7_pfdd_defended_pipeline",
        data_path=uj_path,
        output_dir=output_dir,
        mode="standalone",
        active_families=(PHASE3_V7_PFDD_FAMILY,),
        instrument=InstrumentSpec(symbol="USDJPY", margin_rate=(1.0 / 33.3)),
        spread=SpreadConfig(spread_source="fixed", fixed=FixedSpreadConfig(spread_pips=0.01)),
    ...
    ext["methodology_notes"] = (
        ...
        "Hypothesis selects pipeline L1 spread inside the runner."
    )
