#!/usr/bin/env python3
"""Run Swing-Macro WIDE-STOP backtest: ATR stop 2.5×, trail buffer 1.0× ATR (real spread)."""
...
def main() -> None:
    ...
    output_dir = ROOT / "research_out/swing_macro_wide_real"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    log("=== SWING-MACRO WIDE STOPS — REAL SPREAD (2.5× ATR stop, 1.0× trail buffer) ===")
    ...
    cfg = RunConfig(
        hypothesis="swing_macro_wide_real",
        data_path=uj_path,
    ...
    ext["methodology_notes"] = (
        "WIDE: atr_stop_factor=2.5, trail_buffer_atr=1.0. Fixed 2.0 pip spread + 0.1 slippage."
    )
