...
from scripts import rerun_v44_family_native_hedging_policy as hedging_rerun
...
OUT_JSON = OUT_DIR / 'v44_family_vs_phase3_integrated_validation.json'
OUT_MD = OUT_DIR / 'v44_family_vs_phase3_integrated_validation.md'
PHASE3_BASELINES = {
...
def _offensive_trade_key(trade: dict[str, Any]) -> tuple[str, str, str, str]:
    return (
        ...
        str(pd.Timestamp(trade['entry_time']).tz_convert('UTC') if pd.Timestamp(trade['entry_time']).tzinfo else pd.Timestamp(trade['entry_time']).tz_localize('UTC')),
        str(pd.Timestamp(trade['exit_time']).tz_convert('UTC') if pd.Timestamp(trade['exit_time']).tzinfo else pd.Timestamp(trade['exit_time']).tz_localize('UTC')),
        str(trade.get('side', '')),
    )
...
def _scale_offensive_against_phase3_baseline(
    *,
    baseline_actual_trades: list[merged_engine.TradeRow],
...
def _run_variant_against_phase3(
    *,
    name: str,
...
def _build_md(payload: dict[str, Any]) -> str:
    lines = [
        '# V44 Family Validation Against Phase 3 Integrated Baseline',
        '',
        'This keeps the original `phase3_integrated` baseline trades fixed as reported, then layers the offensive V44 family on top using the same shared-equity style entry scaling for the offensive additions.',
        '',
...
def main() -> int:
    ...
    policy = additive.ConflictPolicy(
        name='phase3_integrated_anchored_hedging_like',
    ...
    v3_selected = hedging_rerun._load_v3_selected(matrix, all_trades)
    v4_selected = hedging_rerun._select_for_specs(hedging_rerun._load_v4_best_specs(matrix), all_trades)
    v5_best_selected = hedging_rerun._load_v5_selected(matrix, all_trades, 'v4_plus_all_adj')
    v5_candidate_a_selected = hedging_rerun._load_v5_selected(matrix, all_trades, 'v5_candidate_A')

    leaderboard = [
        _run_variant_against_phase3(name='v3_full_v3', selected=v3_selected, policy=policy, baselines=baselines),
        _run_variant_against_phase3(name='v4_best', selected=v4_selected, policy=policy, baselines=baselines),
        _run_variant_against_phase3(name='v5_best_current_framework', selected=v5_best_selected, policy=policy, baselines=baselines),
        _run_variant_against_phase3(name='v5_candidate_A', selected=v5_candidate_a_selected, policy=policy, baselines=baselines),
    ]
    leaderboard.sort(key=_score, reverse=True)

    payload = {
        'title': 'V44 family validation against phase3_integrated baseline',
        'policy': {
            'name': policy.name,
