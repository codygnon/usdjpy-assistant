#!/usr/bin/env python3
"""Run Swing-Macro REVERSAL BAR filter backtest — real spread (2.0 pip + 0.1 slippage)."""
...
def main() -> None:
    ...
    output_dir = ROOT / "research_out/swing_macro_reversal_real"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    log("=== SWING-MACRO REVERSAL BAR — REAL SPREAD ===")
    ...
    cfg = RunConfig(
        hypothesis="swing_macro_reversal_real",
        data_path=uj_path,
    ...
    ext["methodology_notes"] = (
        ...
        "Fixed 2.0 pip spread + 0.1 pip slippage. Mid-only USDJPY data."
    )
