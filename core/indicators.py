...
def detect_swing_points(df: pd.DataFrame, lookback: int = 5) -> tuple[list, list]:
    """Detect swing highs and lows using local pivot detection.

    Args:
        df: DataFrame with columns 'high', 'low', 'time'
        lookback: Number of bars on each side to check for pivot

    Returns:
        Tuple of (swing_highs, swing_lows) where each is a list of
        {'time': unix_timestamp, 'price': float}
    """
    swing_highs = []
    swing_lows = []
    ...
    for i in range(lookback, len(df) - lookback):
        ...
        if is_swing_high:
            ...
            swing_highs.append({"time": ts, "price": float(highs[i])})
        ...
        if is_swing_low:
            ts = int(times[i].timestamp()) if hasattr(times[i], "timestamp") else int(times[i])
            swing_lows.append({"time": ts, "price": float(lows[i])})
