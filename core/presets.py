...
PRESETS: dict[PresetId, dict[str, Any]] = {
            ...
            "policies": [
                {
                    ...
                    # Trend exhaustion (M5 stretch), default OFF
                    "trend_exhaustion_enabled": False,
                    "trend_exhaustion_mode": "session_and_side",
                    "trend_exhaustion_use_current_price": True,
                    "trend_exhaustion_hysteresis_pips": 0.5,
                    "trend_exhaustion_p80_global": 12.03,
                    "trend_exhaustion_p90_global": 17.02,
    ...
    # -----------------------------------------------------------------------
    # KT/CG Trial #8 (T7 minus EMA zone + reversal risk; breakeven spread+buffer; zone price vs M1 EMA5; Daily Level Filter)
    # -----------------------------------------------------------------------
    PresetId.KT_CG_TRIAL_8: {
        "name": "KT/CG Trial #8 (Daily Level Filter)",
        "description": "Trial #8: Same as Trial #7 but no EMA zone filter, no reversal risk. Breakeven spread+buffer only; zone entry price vs M1 EMA5 only. Optional Daily Level Filter: block near yesterday/today high/low until 2 closed M5 breakout.",
        "pros": [
            "Simpler than T7: no EMA zone gate, no reversal risk scoring",
            "Daily Level Filter avoids entries stuck at key levels",
            "Spread-relative breakeven keeps BE logic simple",
            "Price vs M1 EMA5 zone entry only = clear zone logic",
        ],
        "cons": [
            "Fewer filters can mean more chop entries if daily filter is off",
            "Testing preset - tune daily level buffer and breakout candles",
        ],
        "risk": {
            "max_lots": 0.1,
            "require_stop": True,
            ...
            "policies": [
                {
                    ...
                    "spread_aware_be_apply_to_zone_entry": True,
                    "spread_aware_be_apply_to_tiered_pullback": True,
                    "use_daily_level_filter": False,
                    "daily_level_buffer_pips": 3.0,
                    "daily_level_breakout_candles_required": 2,
                    "exit_strategy": "tp1_be_trail",
                    "m1_exit_ema_fast": 9,
                    "m1_exit_ema_slow": 21,
                    "scale_out_pct": 50.0,
                    "initial_sl_spread_plus_pips": 5.0,
    ...
    # -----------------------------------------------------------------------
    # KT/CG TRIAL #9 (No-Trade Zones + Kill Switch)
    # Same entries as T8 (M5 trend + zone entry + tiered pullback) but replaces
    # Daily Level Filter with NTZ and adds M1-200 EMA Kill Switch exit.
    # No fixed SL — Kill Switch is the automated risk control for short tiers.
    # -----------------------------------------------------------------------
    PresetId.KT_CG_TRIAL_9: {
        "name": "KT/CG Trial #9 (M5 EMA Trail)",
        "description": "Trial #9: T8 entries + TP1 6p/80% partial + BE 0.5p + M5 EMA-20 bar-close-only trailing exit. NTZ and Kill Switch disabled.",
        "pros": [
            "M5 EMA trail reduces whipsaws vs M1-based trail",
            "Wider TP1 (6p) and higher close pct (80%) captures more profit at target",
            "Tight BE (0.5p) quickly protects runner capital",
            "Bar-close-only trail avoids intrabar noise",
        ],
        "cons": [
            "M5 trail may exit later than M1 trail in reversals",
            "80% TP1 close leaves a smaller runner position",
            "No Kill Switch or NTZ — relies entirely on EMA trail for protection",
        ],
        "risk": {
            "max_lots": 0.1,
            "require_stop": False,
            ...
            "policies": [
                {
                    ...
                    "ntz_use_prev_day_hl": True,
                    "ntz_use_weekly_hl": True,
                    "ntz_use_monthly_hl": True,
                    "ntz_use_fib_pivots": False,
                    "ntz_use_fib_pp": True,
                    "ntz_use_fib_r1": True,
                    "ntz_use_fib_r2": True,
                    "ntz_use_fib_r3": True,
                    ...
                    "conviction_base_lots": 0.05,
                    "conviction_min_lots": 0.01,
                    "kill_switch_enabled": False,
                    "kill_switch_zone_entry_action": "kill",
                    "intraday_fib_enabled": False,
                    "intraday_fib_timeframe": "M15",
                    "intraday_fib_lookback_bars": 16,
                    "intraday_fib_lower_level": "S1",
                    "intraday_fib_upper_level": "R1",
                    "intraday_fib_boundary_buffer_pips": 1.0,
                    "intraday_fib_hysteresis_pips": 1.0,
                    "exit_strategy": "tp1_be_m5_trail",
                    "trail_after_tp1": True,
                    "tp1_pips": 6.0,
                    "tp1_close_pct": 80.0,
                    "be_spread_plus_pips": 0.5,
                    "trail_ema_period": 21,
                    "trail_m5_ema_period": 20,
                    "spread_aware_be_enabled": False,
                    "spread_aware_be_trigger_mode": "spread_relative",
                    "spread_aware_be_spread_buffer_pips": 1.0,
                    "spread_aware_be_apply_to_zone_entry": True,
    ...
    # -----------------------------------------------------------------------
    # KT/CG TRIAL #10 (Proof-Based Continuation)
    # Trial #9 foundation with stricter zone/tier confirmation and tighter caps.
    # -----------------------------------------------------------------------
    PresetId.KT_CG_TRIAL_10: {
        "name": "KT/CG Trial #10 (Proof-Based Continuation)",
        "description": "Trial #10: Trial #9-style entry flow plus runner-score sizing, TP1 tier reset, advisory telemetry, ATR stop, and richer operator audit.",
        "pros": [
            "Runner-score sizing expresses conviction without choking entry flow",
            "TP1 tier reset enables repeated entries in flowing trends",
            "Trial #9-style zone and tier flow remains available to the operator",
            "Spread gate, tier17 floor, and advisory caps still protect the weakest setups",
            "ATR stop, TP1/BE/M5 trail, and dashboard audit remain intact",
        ],
        "cons": [
            "Looser entry flow means more low-quality trades will still appear",
            "Higher trade count means spread drag and total exposure need monitoring",
            "Runner-score lot ladder still needs live validation",
        ],
        "risk": {
            "max_lots": 0.50,
            "require_stop": True,
            ...
            "policies": [
                {
                    ...
                    "m1_zone_entry_ema_slow": 9,
                    "m1_zone_entry_price_ema_period": 5,
                    "zone_entry_price_buffer_pips": 0.5,
                    "zone_entry_require_recent_cross": False,
                    "zone_entry_max_cross_lookback_bars": 2,
                    "tiered_pullback_enabled": True,
                    "tier_ema_periods": [17, 21, 27, 33, 50, 75, 100],
                    "tier_reset_buffer_pips": 1.0,
                    "tier_reclaim_confirmation_enabled": False,
                    "tier_reclaim_ema_period": 5,
                    "strong_m5_only_tier_periods": [50, 75, 100],
                    "shallow_resumption_gate_enabled": False,
                    "sloppy_shallow_block_enabled": False,
                    "pullback_quality_enabled": True,
                    "pullback_quality_lookback_bars": 30,
                    "pullback_quality_orderly_bar_count_min": 6,
                    "pullback_quality_sloppy_bar_count_max": 2,
                    "pullback_quality_orderly_structure_ratio_min": 0.6,
                    "pullback_quality_sloppy_structure_ratio_max": 0.3,
                    "pullback_quality_shallow_tier_periods": [17, 21],
                    "pullback_quality_sloppy_lot_multiplier": 0.5,
                    "regime_gate_enabled": True,
                    "regime_london_sell_veto": False,
                    "regime_london_start_hour_et": 3,
                    "regime_london_end_hour_et": 12,
                    "regime_boost_hours_et": [6, 7, 12, 13, 14, 15],
                    "regime_boost_multiplier": 1.20,
                    "regime_buy_base_multiplier": 0.90,
                    "regime_sell_base_multiplier": 0.75,
                    "regime_chop_pause_enabled": False,
                    "regime_chop_pause_lookback_minutes": 45,
                    "regime_chop_pause_stop_count": 1,
                    "regime_chop_pause_minutes": 45,
                    "tier17_nonboost_multiplier": 0.35,
                    ...
                    "ntz_buffer_pips": 5.0,
                    "ntz_use_prev_day_hl": True,
                    "ntz_use_weekly_hl": False,
                    "ntz_use_monthly_hl": False,
                    "ntz_use_fib_pivots": False,
                    "ntz_use_fib_pp": True,
                    "ntz_use_fib_r1": True,
                    "ntz_use_fib_r2": True,
                    "ntz_use_fib_r3": False,
                    "ntz_use_fib_s1": True,
                    "ntz_use_fib_s2": True,
                    "ntz_use_fib_s3": False,
                    "conviction_sizing_enabled": False,
                    "conviction_base_lots": 0.07,
                    "conviction_min_lots": 0.03,
                    "runner_score_sizing_enabled": True,
                    "runner_bucket_lots_floor": 0.03,
                    "runner_bucket_lots_base": 0.05,
                    "runner_bucket_lots_elevated": 0.07,
                    "runner_bucket_lots_press": 0.15,
                    "runner_bucket_lots_elite": 0.30,
                    ...
                    "runner_exit_buckets": ["press", "elite"],
                    "quick_tp1_pips": 4.0,
                    "quick_tp1_close_pct": 85.0,
                    "quick_be_spread_plus_pips": 0.3,
                    "runner_tp1_pips": 8.0,
                    "runner_tp1_close_pct": 55.0,
                    "runner_be_spread_plus_pips": 0.5,
                    "kill_switch_enabled": False,
                    "kill_switch_zone_entry_action": "hold",
                    "intraday_fib_enabled": False,
                    "intraday_fib_timeframe": "M15",
                    "intraday_fib_lookback_bars": 16,
                    "intraday_fib_lower_level": "S1",
                    ...
                    "trail_escalation_tier1_pips": 10.0,
                    "trail_escalation_tier2_pips": 20.0,
                    "trail_escalation_m15_ema_period": 21,
                    "trail_escalation_h1_ema_period": 9,
                    "trail_escalation_m15_buffer_pips": 1.5,
                    "trail_escalation_h1_buffer_pips": 2.0,
                    "spread_aware_be_enabled": False,
                    "spread_aware_be_trigger_mode": "spread_relative",
                    "spread_aware_be_spread_buffer_pips": 1.0,
                    "spread_aware_be_apply_to_zone_entry": True,
    ...
    PresetId.UNCLE_PARSH_H1_BREAKOUT: {
        "name": "Uncle Parsh Burnout H1 Breakout Scalper Suicidal Run",
        "description": "Major-extremes momentum breakout on USDJPY: YH/YL + weekly/monthly highs/lows -> M5 clean-break -> M1 EMA9/21 entry within 15 minutes. EMA exits: scale-out on EMA9 wrong-side close, runner exit on EMA21 wrong-side close. No broker TP, no BE.",
        "pros": [
            "Structural breakout filtering eliminates weak/false moves",
            "Dual entry modes (Power/Sniper) adapt to market velocity",
            "Scale-out locks profit while letting winners run",
            "Unlimited re-entries at same level - no artificial caps on good setups",
        ],
        "cons": [
            "Fewer trades - requires H1 level + M5 Power Close alignment",
            "Level detection may miss manually-identified zones",
            "Runner portion exposed to sudden reversals between M1 bars",
        ],
        "risk": {
            "max_lots": 0.1,
            "require_stop": True,
        ...
        "trade_management": {
            "target": {"mode": "fixed_pips", "pips_default": 0.0},
            "breakeven": {"enabled": False},
        },
        "execution": {
            "loop_poll_seconds": 1.0,
            ...
            "policies": [
                {
                    ...
                    "h1_min_touches_for_major": 2,
                    "h1_min_distance_between_levels_pips": 10.0,
                    "m5_trend_ema_fast": 9,
                    "m5_trend_ema_slow": 21,
                    "entry_window_minutes": 15,
                    "aggressive_close_distance_pips": 5.0,
                    "m1_entry_ema_fast": 9,
                    "m1_entry_ema_slow": 21,
                    "scale_out_pct": 50.0,
                    "initial_sl_spread_plus_pips": 5.0,
                    "max_spread_pips": 3.0,
                },
