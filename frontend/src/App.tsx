...
interface EditedSettings {
  ...
  t10_regime_london_end_hour_et: number;
  t10_regime_boost_multiplier: number;
  t10_regime_buy_base_multiplier: number;
  ...
  up_m1_entry_ema_slow: number;
  up_scale_out_pct: number;
  up_max_spread_pips: number;
  up_initial_sl_spread_plus_pips: number;
}
...
/** Trial #10: tighter continuation tiers only (17, 21, 27, 33, 50). */
const TRIAL10_DEFAULT_TIER_EMA_PERIODS: number[] = [17, 21, 27, 33, 50];
const TRIAL10_ALLOWED_TIER_EMA_PERIODS = new Set<number>(TRIAL10_DEFAULT_TIER_EMA_PERIODS);

function sanitizeTrial10TierPeriods(periods: number[] | null | undefined): number[] {
  if (!Array.isArray(periods)) return [...TRIAL10_DEFAULT_TIER_EMA_PERIODS];
  const cleaned = Array.from(
    new Set(
      periods
        .map((p) => Math.trunc(Number(p)))
        .filter((p) => TRIAL10_ALLOWED_TIER_EMA_PERIODS.has(p))
    )
  ).sort((a, b) => a - b);
...
function PresetsPage({ profile }: { profile: Profile }) {
  ...
  useEffect(() => {
    if (showActiveSettings && currentProfile) {
      ...
      const policies = execution?.policies as Record<string, unknown>[] | undefined;
      const strategy = currentProfile.strategy as Record<string, unknown> | undefined;
      const filters = strategy?.filters as Record<string, unknown> | undefined;
      const sbb = filters?.session_boundary_block as { enabled?: boolean; buffer_minutes?: number } | undefined;
      const sessionBoundaryBlockEnabled = sbb?.enabled ?? false;
      const sessionBoundaryBlockBufferMinutes = (sbb?.buffer_minutes ?? 15) as number;
      ...
      let t10RegimeLondonEndHourEt = 12;
      let t10RegimeBoostMultiplier = 1.35;
      let t10RegimeBuyBaseMultiplier = 0.65;
      ...
      if (policies) {
        for (const pol of policies) {
          ...
          if (pol.type === 'kt_cg_trial_8' || pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') {
            ...
            tierEmaPeriods = (
              pol.type === 'kt_cg_trial_10'
                ? sanitizeTrial10TierPeriods
                : (pol.type === 'kt_cg_trial_9' ? sanitizeTrial9TierPeriods : sanitizeTrial8TierPeriods)
            )(pol.tier_ema_periods as number[] | undefined);
            policySlPips = (pol.sl_pips as number) ?? 20;
            t7TrendExhaustionEnabled = (pol.trend_exhaustion_enabled as boolean) ?? false;
            t7TrendExhaustionMode = (pol.trend_exhaustion_mode as 'global' | 'session' | 'session_and_side') ?? 'session_and_side';
            t7TrendExhaustionUseCurrentPrice = (pol.trend_exhaustion_use_current_price as boolean) ?? true;
          ...
          if (pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') {
            ...
            t7M1ZoneEntryEmaSlow = (pol.m1_zone_entry_ema_slow as number) ?? 9;
            t8M1ZoneEntryPriceEmaPeriod = (pol.m1_zone_entry_price_ema_period as number) ?? 5;
            tierEmaPeriods = pol.type === 'kt_cg_trial_10'
              ? sanitizeTrial10TierPeriods(pol.tier_ema_periods as number[] | undefined)
              : sanitizeTrial9TierPeriods(pol.tier_ema_periods as number[] | undefined);
            policySlPips = (pol.sl_pips as number) ?? 20;
            t7TrendExhaustionEnabled = (pol.trend_exhaustion_enabled as boolean) ?? false;
            t7TrendExhaustionMode = (pol.trend_exhaustion_mode as 'global' | 'session' | 'session_and_side') ?? 'session_and_side';
            t7TrendExhaustionUseCurrentPrice = (pol.trend_exhaustion_use_current_price as boolean) ?? true;
            ...
            t8BeSpreadPlusPips = (pol.be_spread_plus_pips as number) ?? 2.0;
            t8TrailEmaPeriod = (pol.trail_ema_period as number) ?? 21;
            // Trial #9: read own exit strategy + TP1/BE/Trail fields
            if (pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') {
              t9NtzEnabled = (pol.ntz_enabled as boolean) ?? false;
              t9NtzBufferPips = (pol.ntz_buffer_pips as number) ?? 10.0;
              ...
              t10RegimeLondonEndHourEt = (pol.regime_london_end_hour_et as number) ?? 12;
              t10RegimeBoostMultiplier = (pol.regime_boost_multiplier as number) ?? 1.35;
              t10RegimeBuyBaseMultiplier = (pol.regime_buy_base_multiplier as number) ?? 0.65;
              ...
              t9TrailM5EmaPeriod = (pol.trail_m5_ema_period as number) ?? 20;
            } else {
              t9NtzEnabled = false;
              t9NtzBufferPips = 10.0;
      ...
      const baseEditedSettings: EditedSettings = {
        ...
        t10_regime_london_end_hour_et: tempSettings?.t10_regime_london_end_hour_et ?? t10RegimeLondonEndHourEt,
        t10_regime_boost_multiplier: tempSettings?.t10_regime_boost_multiplier ?? t10RegimeBoostMultiplier,
        t10_regime_buy_base_multiplier: tempSettings?.t10_regime_buy_base_multiplier ?? t10RegimeBuyBaseMultiplier,
        ...
        t9_trail_m5_ema_period: tempSettings?.t9_trail_m5_ema_period ?? t9TrailM5EmaPeriod,
        // Uncle Parsh H1 Breakout (major-extremes momentum breakout)
        ...
        up_m1_entry_ema_slow: ((execution?.policies as Record<string, unknown>[])?.find(p => p.type === 'uncle_parsh_h1_breakout')?.m1_entry_ema_slow as number ?? 21),
  ...
  const handleApplyTemporarySettings = async () => {
    ...
    try {
      ...
      const policies = (execution?.policies as Record<string, unknown>[])?.map(pol => {
        ...
        // Update Trial #8 settings (T7 minus EMA zone + reversal risk; spread+buffer BE only; zone = price_vs_ema5; Daily Level Filter)
        ...
        // Update Trial #9 / Trial #10 settings
        if (pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') {
          updates.m5_trend_ema_fast = Math.max(1, editedSettings.t7_m5_trend_ema_fast);
          updates.m5_trend_ema_slow = Math.max(1, editedSettings.t7_m5_trend_ema_slow);
          updates.m5_trend_source = editedSettings.t7_m5_trend_source;
          ...
          updates.m1_zone_entry_price_ema_period = Math.max(2, Math.min(50, editedSettings.t8_m1_zone_entry_price_ema_period));
          updates.tier_ema_periods = pol.type === 'kt_cg_trial_10'
            ? sanitizeTrial10TierPeriods(editedSettings.tier_ema_periods)
            : sanitizeTrial9TierPeriods(editedSettings.tier_ema_periods);
          updates.max_open_trades_per_side = Math.max(1, editedSettings.max_open_trades_per_side);
          updates.max_zone_entry_open = Math.max(1, editedSettings.max_zone_entry_open);
          updates.max_tiered_pullback_open = Math.max(1, editedSettings.max_tiered_pullback_open);
          updates.sl_pips = editedSettings.policy_sl_pips;
          updates.cooldown_minutes = Math.max(0, editedSettings.policy_cooldown_minutes);
          updates.tp_pips = Math.max(0.1, editedSettings.target_pips);
          updates.trend_exhaustion_enabled = editedSettings.t7_trend_exhaustion_enabled;
          updates.trend_exhaustion_mode = editedSettings.t7_trend_exhaustion_mode;
          ...
          updates.trend_exhaustion_tp_very_extended_offset_pips = Math.max(0, editedSettings.t7_trend_exhaustion_tp_very_extended_offset_pips);
          updates.trend_exhaustion_tp_min_pips = Math.max(0.1, editedSettings.t7_trend_exhaustion_tp_min_pips);
          // T9: Daily Level Filter + Exit Strategy (own t9_* fields)
          updates.use_daily_level_filter = editedSettings.t8_use_daily_level_filter;
          updates.daily_level_buffer_pips = Math.max(0, editedSettings.t8_daily_level_buffer_pips);
        ...
        if (pol.type === 'kt_cg_trial_10') {
            ...
            updates.regime_london_end_hour_et = Math.max(0, Math.min(23, editedSettings.t10_regime_london_end_hour_et));
            updates.regime_boost_multiplier = Math.max(0.1, Math.min(3.0, editedSettings.t10_regime_boost_multiplier));
            updates.regime_buy_base_multiplier = Math.max(0.1, Math.min(2.0, editedSettings.t10_regime_buy_base_multiplier));
            ...
            updates.max_directional_lots_per_side = null;
        ...
        if (pol.type === 'uncle_parsh_h1_breakout') {
          ...
          updates.m1_entry_ema_slow = Math.max(3, Math.min(100, editedSettings.up_m1_entry_ema_slow));
          updates.scale_out_pct = Math.max(10, Math.min(90, editedSettings.up_scale_out_pct));
          updates.max_spread_pips = Math.max(0, editedSettings.up_max_spread_pips);
          updates.initial_sl_spread_plus_pips = Math.max(0, editedSettings.up_initial_sl_spread_plus_pips);
        }
      ...
      const hasTrial7 = policies.some((p: Record<string, unknown>) => p.type === 'kt_cg_trial_7' || p.type === 'kt_cg_trial_8' || p.type === 'kt_cg_trial_9' || p.type === 'kt_cg_trial_10');
      ...
      const newProfile = {
        ...
        ...(hasTrial7 ? {
          strategy: {
            ...
            filters: {
              ...(currentFilters || {}),
              session_boundary_block: {
                enabled: editedSettings.session_boundary_block_enabled,
                buffer_minutes: Math.max(1, Math.min(60, editedSettings.session_boundary_block_buffer_minutes)),
              },
            },
          },
        } : {}),
      ...
      const hasTrial9 = policies.some(p => p.type === 'kt_cg_trial_9' || p.type === 'kt_cg_trial_10');
      if (hasKtCgCtp || hasKtCgTrial4 || hasKtCgTrial5 || hasTrial8 || hasTrial9) {
        ...
        if (hasTrial9) {
          ...
          settings.t10_regime_london_end_hour_et = editedSettings.t10_regime_london_end_hour_et;
          settings.t10_regime_boost_multiplier = editedSettings.t10_regime_boost_multiplier;
          settings.t10_regime_buy_base_multiplier = editedSettings.t10_regime_buy_base_multiplier;
        ...
        <div className="card mb-4" style={{ 
          ...
          {showActiveSettings && risk && (
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
              ...
              {editedSettings && (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_4' || pol.type === 'kt_cg_trial_5' || pol.type === 'kt_cg_trial_7' || pol.type === 'kt_cg_trial_8' || pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') && (
                <>
                ...
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                  ...
                  {/* EMA Zone Entry Filter (Trial #7 only; Trial #8 has no EMA zone filter) */}
                  ...
                  {/* Trial #9: Exit Strategy (separate from T8) */}
                  {(execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') && (
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
                      Trial #9 / #10 Exit Strategy
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12 }}>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        ...
                        <input type="number" step="0.5" min="0.5" value={editedSettings.t9_tp1_pips}
                          onChange={(e) => setEditedSettings({ ...editedSettings, t9_tp1_pips: parseFloat(e.target.value) || 6 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      </div>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        ...
                        <input type="number" step="5" min="10" max="100" value={editedSettings.t9_tp1_close_pct}
                          onChange={(e) => setEditedSettings({ ...editedSettings, t9_tp1_close_pct: parseInt(e.target.value) || 80 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      </div>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        ...
                        <input type="number" step="0.5" min="0" value={editedSettings.t9_be_spread_plus_pips}
                          onChange={(e) => setEditedSettings({ ...editedSettings, t9_be_spread_plus_pips: parseFloat(e.target.value) || 0.5 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      </div>
                      {editedSettings.t9_exit_strategy === 'tp1_be_m5_trail' && (
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Trail M5 EMA period</div>
                        <input type="number" step="1" min="3" max="100" value={editedSettings.t9_trail_m5_ema_period}
                          onChange={(e) => setEditedSettings({ ...editedSettings, t9_trail_m5_ema_period: parseInt(e.target.value) || 20 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      ...
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        ...
                        <input type="number" step="1" min="5" max="50" value={editedSettings.t9_trail_ema_period}
                          ...
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                        <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginTop: 2 }}>M1 bar-close-only trail</div>
                      </div>
                      )}
                    </div>
                    )}
                  ...
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 12 }}>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                          <input type="checkbox" checked={editedSettings.t10_runner_score_sizing_enabled}
                            onChange={(e) => setEditedSettings({ ...editedSettings, t10_runner_score_sizing_enabled: e.target.checked })}
                            style={{ width: 18, height: 18, cursor: 'pointer' }} />
                          <span style={{ fontSize: '0.75rem', fontWeight: 600, color: editedSettings.t10_runner_score_sizing_enabled ? 'var(--success)' : 'var(--text-secondary)' }}>
                            {editedSettings.t10_runner_score_sizing_enabled ? 'Runner Score ON' : 'Runner Score OFF'}
                          </span>
                        </label>
                      </div>
                    </div>
                    {editedSettings.t10_runner_score_sizing_enabled && (
                    <>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12 }}>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Base Lots</div>
                          <input type="number" step="0.01" min="0.01" value={editedSettings.t10_runner_base_lots}
                            onChange={(e) => setEditedSettings({ ...editedSettings, t10_runner_base_lots: parseFloat(e.target.value) || 0.07 })}
                            style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                        </div>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Min Lots</div>
                          <input type="number" step="0.01" min="0.01" value={editedSettings.t10_runner_min_lots}
                            onChange={(e) => setEditedSettings({ ...editedSettings, t10_runner_min_lots: parseFloat(e.target.value) || 0.03 })}
                            style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                        </div>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                  ...
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                    ...
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
                      Operator-style activity filter. London sell veto blocks sells during London hours. Time/side multipliers adjust lot size by ET hour and M5 regime. Chop auto-pause halts a side after consecutive stops.
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 12 }}>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                          <input type="checkbox" checked={editedSettings.t10_regime_gate_enabled}
                            onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_gate_enabled: e.target.checked })}
                            style={{ width: 18, height: 18, cursor: 'pointer' }} />
                          <span style={{ fontSize: '0.75rem', fontWeight: 600, color: editedSettings.t10_regime_gate_enabled ? 'var(--success)' : 'var(--text-secondary)' }}>
                            {editedSettings.t10_regime_gate_enabled ? 'Regime Gate ON' : 'Regime Gate OFF'}
                          </span>
                        </label>
                      </div>
                    </div>
                    {editedSettings.t10_regime_gate_enabled && (
                    <>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 12 }}>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                            <input type="checkbox" checked={editedSettings.t10_regime_london_sell_veto}
                              onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_london_sell_veto: e.target.checked })}
                              style={{ width: 18, height: 18, cursor: 'pointer' }} />
                            <span style={{ fontSize: '0.75rem', fontWeight: 600, color: editedSettings.t10_regime_london_sell_veto ? '#ef4444' : 'var(--text-secondary)' }}>
                              {editedSettings.t10_regime_london_sell_veto ? 'London Sell Veto ON' : 'London Sell Veto OFF'}
                            </span>
                          </label>
                        </div>
                        {editedSettings.t10_regime_london_sell_veto && (
                        <>
                          <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Veto Start (ET)</div>
                            <input type="number" min={0} max={23} step={1} value={editedSettings.t10_regime_london_start_hour_et}
                              onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_london_start_hour_et: parseInt(e.target.value) || 3 })}
                          ...
                          <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Veto End (ET)</div>
                            <input type="number" min={0} max={23} step={1} value={editedSettings.t10_regime_london_end_hour_et}
                              onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_london_end_hour_et: parseInt(e.target.value) || 12 })}
                              style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '0.8rem' }} />
                          </div>
                        </>
                        )}
                      </div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Multipliers</div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12, marginBottom: 12 }}>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Boost (best hrs)</div>
                          <input type="number" min={0.1} max={3.0} step={0.05} value={editedSettings.t10_regime_boost_multiplier}
                            onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_boost_multiplier: parseFloat(e.target.value) || 1.35 })}
                            style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '0.8rem' }} />
                        </div>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Buy Base</div>
                          <input type="number" min={0.1} max={2.0} step={0.05} value={editedSettings.t10_regime_buy_base_multiplier}
                            onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_buy_base_multiplier: parseFloat(e.target.value) || 0.65 })}
                            style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '0.8rem' }} />
                        </div>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Sell Base</div>
                          <input type="number" min={0.1} max={2.0} step={0.05} value={editedSettings.t10_regime_sell_base_multiplier}
                            onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_sell_base_multiplier: parseFloat(e.target.value) || 0.35 })}
                            style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '0.8rem' }} />
                        </div>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Tier 17 Non-Boost</div>
                          <input type="number" min={0.1} max={2.0} step={0.05} value={editedSettings.t10_tier17_nonboost_multiplier}
                            onChange={(e) => setEditedSettings({ ...editedSettings, t10_tier17_nonboost_multiplier: parseFloat(e.target.value) || 0.35 })}
                            style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '0.8rem' }} />
                        </div>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 12 }}>
                        <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                            <input type="checkbox" checked={editedSettings.t10_regime_chop_pause_enabled}
                              onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_chop_pause_enabled: e.target.checked })}
                              style={{ width: 18, height: 18, cursor: 'pointer' }} />
                            <span style={{ fontSize: '0.75rem', fontWeight: 600, color: editedSettings.t10_regime_chop_pause_enabled ? 'var(--warning)' : 'var(--text-secondary)' }}>
                              {editedSettings.t10_regime_chop_pause_enabled ? 'Chop Pause ON' : 'Chop Pause OFF'}
                            </span>
                          </label>
                        </div>
                        {editedSettings.t10_regime_chop_pause_enabled && (
                        <>
                          <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Pause (min)</div>
                            <input type="number" min={5} max={120} step={5} value={editedSettings.t10_regime_chop_pause_minutes}
                              onChange={(e) => setEditedSettings({ ...editedSettings, t10_regime_chop_pause_minutes: parseInt(e.target.value) || 45 })}
                              style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '0.8rem' }} />
                          </div>
                          <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                  ...
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
                      Trial #9 / #10 Intraday Fibonacci Corridor
                    </div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
                      ...
                      Optional structural filter for Trial #9 / #10; when enabled it can keep entries inside a cleaner continuation corridor.
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 12 }}>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  ...
                  {/* Reversal Risk (Trial #7) — directly under Trend Exhaustion; expands when ON */}
                  {(execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_7') && (
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
                    ...
                    <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6, marginBottom: 10 }}>
                      ...
                      <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginTop: 4 }}>
                        Score: RSI divergence, ADR exhaustion, HTF proximity, EMA spread. Reduces size / blocks at medium/high/critical. Turn ON to show options.
                      </div>
                    </div>
              ...
              {/* View Settings (for Trial #4 / Trial #5 / Trial #7 / Trial #8 / Trial #9 / Trial #10) */}
              {editedSettings && (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_4' || pol.type === 'kt_cg_trial_5' || pol.type === 'kt_cg_trial_7' || pol.type === 'kt_cg_trial_8' || pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: 12 }}>
                    {(execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_7')
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
                    {!(execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_7' || pol.type === 'kt_cg_trial_8' || pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') && (
                    <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
                    ...
                    <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                      ...
                      {((execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_7') || (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_8') || (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_9') || (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_10')) && (
                        <div style={{ marginTop: 10 }}>
                          ...
                          {editedSettings.t7_zone_entry_mode === 'price_vs_ema5' && (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_8' || pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') && (
                            <div style={{ marginTop: 8 }}>
                    ...
                    <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                      {((execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_8') || (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_9') || (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_10')) ? (
                        <>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
                            Tiered Pullback — {(execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_10') ? 'Trial #10' : ((execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_9') ? 'Trial #9' : 'Trial #8')}
                          </div>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                            {(
                              (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_10')
                                ? TRIAL10_DEFAULT_TIER_EMA_PERIODS
                                : ((execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_9') ? TRIAL9_DEFAULT_TIER_EMA_PERIODS : TRIAL8_DEFAULT_TIER_EMA_PERIODS)
                            ).map(tier => (
                              <label key={tier} style={{ display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer', padding: '2px 6px', background: editedSettings.tier_ema_periods.includes(tier) ? 'var(--success)' : 'var(--bg-secondary)', borderRadius: 4, fontSize: '0.7rem', fontWeight: 600, color: editedSettings.tier_ema_periods.includes(tier) ? '#fff' : 'var(--text-secondary)' }}>
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setEditedSettings({
                                      ...editedSettings,
                                      tier_ema_periods: (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_10')
                                        ? sanitizeTrial10TierPeriods(periods)
                                        : ((execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_9') ? sanitizeTrial9TierPeriods(periods) : sanitizeTrial8TierPeriods(periods)),
                                    });
                                  }}
                                  style={{ width: 14, height: 14, cursor: 'pointer' }}
                                />
                    ...
                    {(execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'kt_cg_trial_5' || pol.type === 'kt_cg_trial_7' || pol.type === 'kt_cg_trial_8' || pol.type === 'kt_cg_trial_9' || pol.type === 'kt_cg_trial_10') && (
                    <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
                    ...
                    <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                          <input
                            type="checkbox"
                            checked={editedSettings.session_boundary_block_enabled}
                            onChange={(e) => setEditedSettings({ ...editedSettings, session_boundary_block_enabled: e.target.checked })}
                            style={{ width: 18, height: 18, cursor: 'pointer' }}
                          />
                          <span style={{ fontWeight: 600, color: editedSettings.session_boundary_block_enabled ? 'var(--success)' : 'var(--text-secondary)' }}>
                            {editedSettings.session_boundary_block_enabled ? 'ON' : 'OFF'}
                          </span>
                        </label>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Buffer (min):</span>
                          <input
                            type="number"
                            step="1"
                            min="1"
                            max="60"
                            value={editedSettings.session_boundary_block_buffer_minutes}
                            onChange={(e) => setEditedSettings({ ...editedSettings, session_boundary_block_buffer_minutes: Math.max(1, Math.min(60, parseInt(e.target.value) || 15)) })}
                            style={{ width: 56, padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }}
                          />
                        </div>
                      </div>
                      <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginTop: 8 }}>
              ...
              {editedSettings && (execution?.policies as Record<string, unknown>[])?.some(pol => pol.type === 'uncle_parsh_h1_breakout') && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12, marginBottom: 12 }}>
                    <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Entry Window (minutes)</div>
                      <input type="number" step="1" min="1" max="60" value={editedSettings.up_entry_window_minutes}
                        onChange={(e) => setEditedSettings({ ...editedSettings, up_entry_window_minutes: parseInt(e.target.value) || 15 })}
                        style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                    </div>
                    <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Aggressive Threshold (pips)</div>
                      <input type="number" step="0.5" min="0.5" max="20" value={editedSettings.up_aggressive_close_distance_pips}
                        onChange={(e) => setEditedSettings({ ...editedSettings, up_aggressive_close_distance_pips: parseFloat(e.target.value) || 5 })}
                        style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginTop: 2 }}>M5 break close distance past level</div>
                    </div>
                  </div>

                  <div style={{ marginTop: 8, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 8, fontWeight: 600 }}>M1 EMA (Entry + Exit)</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>EMA Fast (default 9)</div>
                        <input type="number" step="1" min="2" max="50" value={editedSettings.up_m1_entry_ema_fast}
                          onChange={(e) => setEditedSettings({ ...editedSettings, up_m1_entry_ema_fast: parseInt(e.target.value) || 9 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      </div>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>EMA Slow (default 21)</div>
                        <input type="number" step="1" min="3" max="100" value={editedSettings.up_m1_entry_ema_slow}
                          onChange={(e) => setEditedSettings({ ...editedSettings, up_m1_entry_ema_slow: parseInt(e.target.value) || 21 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      </div>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Scale-out % (EMA fast wrong side)</div>
                        <input type="number" step="5" min="10" max="90" value={editedSettings.up_scale_out_pct}
                          onChange={(e) => setEditedSettings({ ...editedSettings, up_scale_out_pct: parseInt(e.target.value) || 50 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      </div>
                    </div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginTop: 8 }}>
                      Scale-out on EMA fast wrong-side close. Runner closes on EMA slow wrong-side close. No break-even moves and no broker TP.
                    </div>
                  </div>

                  <div style={{ marginTop: 8, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 8, fontWeight: 600 }}>Risk / Discipline</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Max Spread (pips)</div>
                        <input type="number" step="0.5" min="0" max="10" value={editedSettings.up_max_spread_pips}
                          onChange={(e) => setEditedSettings({ ...editedSettings, up_max_spread_pips: parseFloat(e.target.value) || 3 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      </div>
                      <div style={{ padding: 8, background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4 }}>Initial SL: spread + pips</div>
                        <input type="number" step="0.5" min="0" max="20" value={editedSettings.up_initial_sl_spread_plus_pips}
                          onChange={(e) => setEditedSettings({ ...editedSettings, up_initial_sl_spread_plus_pips: parseFloat(e.target.value) || 5 })}
                          style={{ width: '100%', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 600 }} />
                      </div>
                    </div>
                  </div>
...
function AdvancedAnalytics({ profileName, profilePath }: { profileName: string; profilePath: string }) {
  ...
  type BreakdownRow = { label: string; trades: number; wins: number; winRate: number; avgPnl: number | null };
  function computeBreakdown(
    arr: api.AdvancedTrade[],
    key: (t: api.AdvancedTrade) => string | null,
    order?: string[],
  ): BreakdownRow[] {
    const groups: Record<string, api.AdvancedTrade[]> = {};
    for (const t of arr) {
      const k = key(t) ?? 'Unknown';
      if (!groups[k]) groups[k] = [];
      groups[k].push(t);
    }
    const labels = order ? order.filter(l => groups[l]) : Object.keys(groups).sort();
    return labels.map(label => {
      const g = groups[label] || [];
      const wins = g.filter(t => (t.pips ?? 0) > 0).length;
      const profits = g.map(t => t.profit).filter((p): p is number => p != null);
      return {
        label,
        trades: g.length,
        wins,
        winRate: g.length > 0 ? wins / g.length : 0,
        avgPnl: profits.length > 0 ? profits.reduce((a, b) => a + b, 0) / profits.length : null,
      };
    });
  }

  function getSession(t: api.AdvancedTrade): string {
    try {
      const h = new Date(t.entry_time_utc).getUTCHours();
  ...
  const breakdownData = useMemo(() => {
    if (!expandedSections.breakdown) return null;
    const DAYS = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    return {
      entryType: computeBreakdown(filtered, t => t.entry_type, ['zone_entry', 'tiered_pullback']),
      tier: computeBreakdown(filtered, t => t.tier_number != null ? `EMA ${t.tier_number}` : null),
      session: computeBreakdown(filtered, getSession, ['Tokyo', 'London', 'New York']),
      riskTier: computeBreakdown(filtered, t => t.reversal_risk_tier, ['low', 'medium', 'high', 'critical']),
      dayOfWeek: computeBreakdown(filtered, t => { try { return DAYS[new Date(t.entry_time_utc).getUTCDay()]; } catch { return null; } }, DAYS),
      hourOfDay: computeBreakdown(filtered, t => { try { return `${new Date(t.entry_time_utc).getUTCHours().toString().padStart(2, '0')}:00`; } catch { return null; } }),
    };
  }, [expandedSections.breakdown, filtered]);
  ...
  return (
    <div className="card mb-4">
      ...
      {/* Feature 6: Breakdown Analysis */}
      <div style={sectionHeaderStyle} onClick={() => toggleSection('breakdown')}>
        <strong>Breakdown Analysis</strong>
        <span>{expandedSections.breakdown ? '▼' : '▶'}</span>
      </div>
      {expandedSections.breakdown && (
        <div style={{ padding: '12px 0' }}>
          {breakdownData == null ? (
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', padding: '0 16px' }}>Loading...</p>
          ) : (() => {
            const tableStyle: React.CSSProperties = { width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' };
            const thStyle: React.CSSProperties = { padding: '6px 10px', textAlign: 'left', borderBottom: '1px solid var(--border)', color: 'var(--text-secondary)', fontWeight: 600 };
            const tdStyle: React.CSSProperties = { padding: '5px 10px', borderBottom: '1px solid var(--border)' };
            const tdNumStyle: React.CSSProperties = { ...tdStyle, textAlign: 'right' };
            const winColor = (wr: number) => wr >= 0.55 ? 'var(--success)' : wr >= 0.4 ? 'var(--warning)' : 'var(--danger)';
            const pnlColor = (v: number | null) => v == null ? 'var(--text-secondary)' : v > 0 ? 'var(--success)' : v < 0 ? 'var(--danger)' : 'var(--text-secondary)';
            const fmtPnl = (v: number | null) => v == null ? '—' : (v >= 0 ? '+' : '') + v.toFixed(2);
            ...
            function BreakdownTable({ title, rows }: { title: string; rows: BreakdownRow[] }) {
              if (rows.length === 0) return (
                <div style={{ marginBottom: 24 }}>
                  ...
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>No data</p>
                </div>
              );
              return (
                <div style={{ marginBottom: 24 }}>
                  ...
                  <table style={tableStyle}>
                    <thead>
                      <tr>
                        <th style={thStyle}>{title.split(' ')[0]}</th>
                    ...
                    <tbody>
                      {rows.map(r => (
                        <tr key={r.label}>
                          ...
                          <td style={tdNumStyle}>{r.trades}</td>
                          <td style={{ ...tdNumStyle, color: winColor(r.winRate), fontWeight: 600 }}>{(r.winRate * 100).toFixed(1)}%</td>
                          <td style={{ ...tdNumStyle, color: pnlColor(r.avgPnl) }}>{fmtPnl(r.avgPnl)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            return (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24, padding: '4px 0' }}>
                <BreakdownTable title="Entry Type" rows={breakdownData.entryType} />
                <BreakdownTable title="Tier Number" rows={breakdownData.tier} />
                <BreakdownTable title="Session" rows={breakdownData.session} />
                <BreakdownTable title="Risk Tier" rows={breakdownData.riskTier} />
                <BreakdownTable title="Day of Week" rows={breakdownData.dayOfWeek} />
                <BreakdownTable title="Hour of Day (UTC)" rows={breakdownData.hourOfDay} />
              </div>
            );
          })()}
        </div>
